  ...
  while ((c = getopt_long(argc, argv, "m:l:t:f:i:hT", longOpts,
    ...
    switch (c) {
      ...
      case 'T':
        ...
        KAYLORDUT_LOG_INFO("Track2 {}", options.is_track);
        break;
      case '?':
...
int main(int argc, char *argv[]) {
  ...
  const std::string output_video_path = "output_video.avi"; // 输出视频路径
  int video_fps = static_cast<int>(options.framerate);
  cv::Size video_size(video_file.get_frame_width(), video_file.get_frame_height()); 
    ...
    auto func = [&] {
      ...
      if (image_res != nullptr) {
        ...
        if (!video_writer.isOpened()) {
          ...
          video_writer.open(
              output_video_path,
              cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), // 编码格式
              video_fps,
              video_size
          );
          if (!video_writer.isOpened()) {
            KAYLORDUT_LOG_ERROR("Failed to open video writer!");
            return;
          }
        }
        video_writer.write(*image_res); // 写入当前帧
        // ================================
  ...
  if (video_writer.isOpened()) {
    ...
    KAYLORDUT_LOG_INFO("Video saved to: {}", output_video_path);
  }
