...
ImageProcess::ImageProcess(int width, int height, int target_size,
  ...
  is_track_ = is_track;
  KAYLORDUT_LOG_INFO("Track1 {}", is_track_);
  KAYLORDUT_LOG_INFO("Track0 {}", is_track);
  if (is_track) {
...
void ImageProcess::ImagePostProcess(cv::Mat &image,
  ...
  if (od_results.model_type == ModelType::DETECTION || 
    ...
    od_results.model_type == ModelType::POSE) {
    
    if (is_track_) {
        ProcessPoseAndTrackImage(image, od_results);
        // ProcessTrackImage(image, od_results);
        KAYLORDUT_LOG_INFO("Tracking!");
    } else {
        if (od_results.model_type == ModelType::POSE) {
  ...
  // if (od_results.model_type == ModelType::DETECTION || od_results.model_type == ModelType::V10_DETECTION||od_results.model_type == ModelType::POSE) {
  //   if (is_track_) {
  //     ProcessTrackImage(image, od_results);
  //     KAYLORDUT_LOG_INFO("Tracking!");
  //   } else {
...
void ImageProcess::ProcessTrackImage(cv::Mat &image,
                                     object_detect_result_list &od_results) {
  KAYLORDUT_LOG_INFO("Track is called");
...
void ImageProcess::ProcessPoseAndTrackImage(cv::Mat &image, object_detect_result_list &od_results) {
    KAYLORDUT_LOG_INFO("Pose Detection and Tracking is called");
    ...
    for (int i = 0; i < od_results.count; ++i) {
        ...
        for (int j = 0; j < 17; ++j) {
            ...
            points.at(j) = cv::Point(od_results.results_pose[i].kpt[j * 2 + 0],
                                      od_results.results_pose[i].kpt[j * 2 + 1]);
            cv::circle(image, points.at(j), 5, cv::Scalar(0, 0, 255), cv::FILLED, cv::LINE_AA);
        }

        // 画骨架
        std::vector<int> pairs = {
            0,  1,  1,  3,  0,  2,  2,  4,  0,  5,  5,  7,  7,  9,  
            0,  6,  6,  8,  8,  10, 5,  6,  11, 12, 11, 5,  12, 6,  
    ...
    // Step 2: 将检测到的人体框送入追踪器
    tracker_mutex_.lock();
    std::vector<STrack> output_stracks = tracker_->update(objects);
    tracker_mutex_.unlock();

    // Step 3: 追踪人体框，并给它们赋予 ID
    for (size_t i = 0; i < output_stracks.size(); ++i) {
        std::vector<float> tlwh = output_stracks[i].tlwh;
        bool vertical = tlwh[2] / tlwh[3] > 1.6;
        if (tlwh[2] * tlwh[3] > 20 && !vertical) {
            Scalar s = tracker_->get_color(output_stracks[i].track_id);
            putText(image, format("%d", output_stracks[i].track_id),
                    Point(tlwh[0], tlwh[1] - 5), 0, 0.6, Scalar(0, 255, 0), 2, LINE_AA);
            rectangle(image, Rect(tlwh[0], tlwh[1], tlwh[2], tlwh[3]), s, 2);
        }
    }
}
